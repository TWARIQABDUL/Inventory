...
export default function Inventory() {
  ...
  const handleEditSubmit = async (values) => {
    try {
      ...
      if (res.ok) {
        ...
        if (priceRes.ok) message.success("Price updated successfully!");
        else {
  ...
  const columns = [
    ...
    { title: "Product Category", dataIndex: "categoryName", key: "categoryName", render: (cat) => <Tag color="purple">{cat}</Tag> },
  ...
  const steps = [
    {
      ...
      content: (
        <Form form={form} layout="vertical" onFinish={handleSubmit}>
          ...
          <Form.Item name="description" label="Description" rules={[{ required: true }]}><Input.TextArea rows={3} placeholder="Enter product description" /></Form.Item>
    ...
    },
    {
      ...
      content: (
        <Form form={form} layout="vertical" onFinish={handleSubmit}>
          <Form.Item name="price" label="Price" rules={[{ required: true }]}><InputNumber min={0} step={0.01} style={{ width: "100%" }} /></Form.Item>
          <Button type="primary" htmlType="submit">Submit Price</Button>
          <Button style={{ marginLeft: 8 }} onClick={() => dispatch({ type: "SET_STEP", payload: state.currentStep - 1 })}>Previous</Button>
        </Form>
      ),
    },
    {
  ...
  return (
    <div style={{ padding: 20, background: "#f5f7fa", minHeight: "100vh" }}>
      ...
      <Modal title="Add Category" open={state.isCategoryModalVisible} onCancel={() => dispatch({ type: "TOGGLE_CATEGORY_MODAL", payload: false })} footer={null}>
        <Form form={categoryForm} layout="vertical" onFinish={async (values) => {
          try {
            const res = await fetch(`${baseUrl}/category`, {
              method: "POST",
              headers: { "Content-Type": "application/json" },
            ...
            if (res.ok) {
              ...
              dispatch({ type: "SET_CATEGORIES", payload: [...state.categories, data] });
          ...
          <Form.Item label="Category Name" name="categoryName" rules={[{ required: true, message: "Please enter category name" }]}><Input /></Form.Item>
          <Form.Item><Button type="primary" htmlType="submit" block>Add Category</Button></Form.Item>
        </Form>
      </Modal>

      <Modal title="Add Inventory" open={state.isModalVisible} onCancel={() => { dispatch({ type: "TOGGLE_MODAL", payload: false }); dispatch({ type: "SET_STEP", payload: 0 }); form.resetFields(); }} footer={null}>
